...
def test_property(under_test_read_only: PropertyWrapper, default_property_base: Property):
...
def test_property_get(under_test_read_only: PropertyWrapper, default_property_base: Property):
    assert (not default_property_base._lock.locked())
    assert (under_test_read_only.get() == DEFAULT_PROPERTY_VALUE)


def test_property_no_read(under_test_nothing: PropertyWrapper, default_property_base: Property):
    assert (not default_property_base._lock.locked())
...
def test_property_illegal_push(context_mock):
    prop_no_push = Property(name=DEFAULT_PROPERTY_NAME, default_value=DEFAULT_PROPERTY_VALUE, allow_push=False)
...
def test_property_illegal_pop(context_mock):
    prop_no_pop = Property(name=DEFAULT_PROPERTY_NAME, default_value=DEFAULT_PROPERTY_VALUE, allow_pop=False)
    ...
    assert wrapper.push(child=Property(name=CHILD_PROPERTY_NAME))
    with LogCapture(attributes=strip_prefix) as log_capture:
