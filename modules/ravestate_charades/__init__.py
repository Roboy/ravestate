...
if ROS_AVAILABLE:
    ...
    with rs.Module(name="charades",
        ...
        def stop_game_session(ctx: rs.ContextWrapper):
            '''
            go off the game session if idle, so that a new game
            can be trigerred if the previous session is abandoned
            '''
        ...
        @rs.state(cond=nlp.prop_yesno.changed() & sig_offered_charades,
                  read=(nlp.prop_yesno, prop_game_in_progress),
                  write=(rawio.prop_out, prop_choice_unclear),
                  signal=sig_play_decision_asked, emit_detached=True)
        def process_play_decision(ctx: rs.ContextWrapper):
            if not ctx[prop_game_in_progress]:
                if ctx[nlp.prop_yesno].yes():
                    ctx[rawio.prop_out] = "Cool." \
        ...
        @rs.state(cond=(nlp.prop_yesno.changed() &
                        sig_play_decision_asked.max_age(10)),
                  read=(nlp.prop_yesno, prop_game_in_progress),
        ...
        def offer_activity_choice(ctx: rs.ContextWrapper):
            ctx[rawio.prop_out] = "Please choose one activity that you " \
                "will show to me. Once you choose it say 'start'"
            return rs.Emit()
        ...
        @rs.state(cond=(sig_activity_choice_prompted.min_age(16).max_age(-1) |
                        (sig_readiness_check_started &
                         prop_choice_unclear.changed())),
                  write=rawio.prop_out,
                  signal=sig_activity_choice_prompted,
                  emit_detached=True)
        def ping_actvity_choice(ctx: rs.ContextWrapper):
            ctx[rawio.prop_out] = "Have you chosen anything? " \
        ...
        def name_the_label(ctx: rs.ContextWrapper):
            ...
            if message:
                ctx[rawio.prop_out] = message.label
                ctx[prop_waiting_for_label] = False
                return rs.Emit()
            else:
                ctx[rawio.prop_out] = BROKEN_MESSAGE
        ...
        @rs.state(cond=(nlp.prop_yesno.changed() &
                        (sig_label_named | prop_choice_unclear.changed())),
                  write=(rawio.prop_out, prop_choice_unclear),
                  signal=sig_round_completed,
                  emit_detached=True)
        def react_to_feedback(ctx: rs.ContextWrapper):
            if ctx[nlp.prop_yesno].yes():
                ...
                ctx[rawio.prop_out] = "I did not understand what you said. " \
                    "Did I guess correctly?"
                ctx[prop_choice_unclear] = True

        @rs.state(cond=sig_round_completed,
                  write=rawio.prop_out,
        ...
        @rs.state(cond=nlp.prop_yesno.changed() & sig_asked_to_continue,
                  write=(rawio.prop_out, prop_game_in_progress),
        ...
        def react_to_continue_decision(ctx: rs.ContextWrapper):
            if ctx[nlp.prop_yesno].yes():
                ...
                ctx[rawio.prop_out] = "I will count that as yes. " \
                    "Let's continue."
                return rs.Emit()
