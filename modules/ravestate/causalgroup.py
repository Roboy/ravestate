...
class CausalGroup:
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> bool:
        if self._locked_lock:
            ...
            return result
        console.error(f"Exit called before enter causal group!")
        return True
    ...
    def merge(self, other: 'CausalGroup'):
        ...
        allowed_propnames = self._refc_index.keys() & other._refc_index.keys()
        self.consumed(self._refc_index.keys() - allowed_propnames)
        other.consumed(other._refc_index.keys() - allowed_propnames)
        # Retarget other's members
        other._lock = self._lock
        other._unwritten_props = self._unwritten_props
        other._refc_index = self._refc_index
    ...
    def acquired(self, sig: 'ISignalInstance', acquired_by: IStateActivation) -> bool:
        ...
        for prop in acquired_by.write_props():
            if prop not in self._refc_index:
                return False
        for prop in acquired_by.write_props():
            self._refc_index[prop][sig][acquired_by] += 1
        return True
    ...
    def rejected(self, sig: 'ISignalInstance', rejected_by: IStateActivation) -> None:
        ...
        for prop in rejected_by.write_props():
            if prop in self._refc_index and \
                sig in self._refc_index[prop] and \
                rejected_by in self._refc_index[prop][sig]:
                remaining_refc = self._refc_index[prop][sig][rejected_by]
                if remaining_refc > 0:
                    self._refc_index[prop][sig][rejected_by] -= 1
                    if remaining_refc > 1:
                ...
                del self._refc_index[prop][sig][rejected_by]
                if len(self._refc_index[prop][sig]) == 0:
                    del self._refc_index[prop][sig]
    ...
    def pressure_activation(self, ready_suitor: IStateActivation) -> bool:
        ...
        """
        # Gather candidate state activations that compete with ready_suitor
        candidates = []
        specificity = ready_suitor.specificity()
        for prop in rejected_by.write_props():
            if prop in self._unwritten_props:
                for sig in self._refc_index[prop]:
                    for candidate in self._refc_index[prop][sig]:
        ...
        for prop in rejected_by.write_props():
            del self._refc_index[prop][ready_suitor]
        # Mark the consented w-props as unavailable
        self._unwritten_props -= rejected_by.write_props()
        return True
    ...
    def consumed(self, consumed_props: Set[str]) -> None:
        ...
        for prop in consumed_props:
            ...
            for sig in self._refc_index[prop]:
                for act in self._refc_index[prop][sig]:
                    act.wiped(sig)
            # Remove the consumed prop from the index
            del self._refc_index[prop]
    ...
    def wiped(self, sig: 'ISignalInstance') -> None:
        ...
        """
        for prop in self._refc_index:
            if sig in self._refc_index[prop]:
                for act in self._refc_index[prop][sig]:
                    act.wiped(sig)
                del self._refc_index[prop][sig]
    ...
    def stale(self, sig: 'ISignalInstance') -> bool:
        ...
        """
        return not any(
            len(self._refc_index[prop][sig]) > 0
            for prop in self._refc_index
            if sig in self._refc_index[prop])
