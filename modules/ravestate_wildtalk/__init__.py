...
with rs.Module(name="wildtalk"):
    ...
    tokenizer_class = GPT2Tokenizer if "gpt2" ==  model else OpenAIGPTTokenizer
    tokenizer = tokenizer_class.from_pretrained( model_checkpoint)
    model_class = GPT2LMHeadModel if "gpt2" ==  model else OpenAIGPTLMHeadModel
    ...
    # personalities = get_dataset_personalities(tokenizer,  dataset_path,  dataset_cache)
    # personality = random.choice(personalities)
    rave_personality = ["i am a rave bot and i love parties.", "i have a party this friday.", "my party will have lasers.", "i will dance until the sunrise.", "techno is my favorite kind of music.", "i organize a rave"]
    personality = []
    for p in rave_personality:
        personality.append(tokenizer.encode(p))
    ...
    def wildtalk_state(ctx):
        ...
        if not out_text:  # make sure that text is not empty
            return rs.Resign()
        result = fix_spaces.sub(lambda x: "{} ".format(x.group(1).replace(" ", "")), out_text)
